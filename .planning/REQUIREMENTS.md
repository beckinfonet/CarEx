...
- [x] **NDIG-01**: An in-process `node-cron` job runs in the Express service, gated by `require.main === module` so tests don't start it.
...
- [x] **NDIG-05**: The cron prunes dead device tokens and notifications older than 90 days (satisfies NDOM-06).
