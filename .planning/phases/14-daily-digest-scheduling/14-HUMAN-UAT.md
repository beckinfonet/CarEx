---
status: passed
phase: 14-daily-digest-scheduling
source: [14-VERIFICATION.md]
started: 2026-06-07
updated: 2026-06-07
---

## Current Test

[complete]

## Tests

### 1. Digest push tap routes to the Notification Center (D-03 / NDIG-03)
expected: Seed a `digestPending` Notification row for your test user, trigger `runDigest` on the deployed backend (or wait for the 08:00 Asia/Bishkek cron), then tap the resulting digest push. From all three app states — foreground, background, and quit (cold start) — the app opens the in-app Notification Center (`NotificationsScreen`), NOT CarDetails or any other screen.
result: pass
notes: Operator verified 2026-06-07 on a real device (Xcode dev build) against deployed backend `main`. Live end-to-end run confirmed `runDigest result: { claimed: 1, users: 1, sent: 1, cleared: 1 }`, push delivered (1 registered device token), and the tap opened the in-app Notification Center. RU fallback title rendered correctly («1 новая машина»).

## Summary

total: 1
passed: 1
issues: 0
pending: 0
skipped: 0
blocked: 0

## Gaps
