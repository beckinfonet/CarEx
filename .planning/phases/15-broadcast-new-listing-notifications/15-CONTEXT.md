...
### Post-Research Resolutions (2026-06-10, after gsd-phase-researcher + user confirmation)
- **R-01 (overrides D-04 cap VALUE):** The broadcast cap limit is **driven by the user's existing `notificationPrefs.dailyCap` preference value (model default `3`)** — NOT a fixed `5` constant. Confirmed by user against research finding A1 (the code's `dailyCap` default is 3, not the SPEC's 5; SPEC req 4 wording "driven by the user's existing dailyCap preference value where set" wins). The **dedicated-counter mechanism from D-05 still stands** — only broadcast pushes count against this budget (via the `pushSuppressed`/sent-marker), so saved-search/watch alerts never consume it and vice-versa. The eligible-user query must therefore `select` `notificationPrefs.dailyCap` so the per-user limit is available in the broadcast loop; fall back to `3` when unset. There is NO new `BROADCAST_DAILY_CAP=5` constant.
- **R-02 (latent-bug fix, in-scope):** Fix `PUT /api/users/:uid` (`backend server.js:~542`) to **persist the whole `notificationPrefs` sub-object** (allowlist + type-check known keys: `muteAll/savedSearchEnabled/watchEnabled/newListingEnabled/quietHours/dailyCap`, dot-path `$set` so a partial patch never clobbers siblings). The handler's allowlist currently omits `notificationPrefs` entirely, so the EXISTING `savedSearchEnabled`/`watchEnabled` toggles are silently dropped today — this fix retroactively repairs them too. Confirmed in-scope by user (required for SPEC req 5 / D-11 to function at all). Keep the IDOR guard: the update keys on `req.params.uid`/verified caller, never a body `uid`.
